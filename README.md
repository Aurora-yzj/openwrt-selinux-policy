...
In this example we're going to demonstrate OpenWrt SELinux policy
customization and testing. If you intent to deploy your own customized
version of `selinux-policy` to your device, or if you intent to help
improve the `selinux-policy` models provided by OpenWrt then you
should familiarize yourself with this procedure.

## Prequisites

This example uses Fedora 34 GNU/Linux for image building and there is
atleast twenty Gigabytes of storage available on the be build system.
The policy will be deployed and tested on a Linksys WRT1900ACS router.
In addition to the above we have access to Git repository that can be
accessed with the `https` protocol. This can be private Git repository
or a public service such as GitLab or GitHub.
...
dependencies installed on our build system.
```
...
our requirements. It has to have support for SELinux and our Linksys
WRT1900ACS target.
```
...
clean factory install using the created `~/openwrt/bin/targets//mvebu/cortexa9/openwrt-mvebu-cortexa9-linksys_wrt1900acs-squashfs-factory.img`
...
all modules so that you can have a good idea if and how it works in
...
dependencies on other modules. It is adviced that you test locally
whether all dependencies of your selection of modules can be resolved.

Depending on how integrated to component you want to target it is wise
you set the default mode to "permissive" during the development phase.
Eventhough that does not apply to this example we will default to
permissive mode for now for the sake of setting a good example.
...
the upstream `selinux-policy`, simply consilidate the two, and push
"myfork" to GitHub.
```
...
We would like to build the whole policy minus `sandbox.cil` module.
To achieve this we will add a target to the
`~/selinux-policy-myfork/Makefile` that can be used to achieve the
desired goal. Before pusing the result to GitHUb we will ensure that
the policy builds. Open `~/selinux-policy-myfork/Makefile` and make
...
[kcinimod@brutus openwrt]$ make -j$((`nproc` + 1)) menuconfig
```
Now we'll enable selinux-policy-myfork in a "Base system" sub menu.
```
    Base system  --->
...
[kcinimod@brutus openwrt-imagebuilder-mvebu-cortexa9.Linux-x86_64]$ make image PACKAGES="/home/kcinimod/openwrt/bin/packages/arm_cortex-a9_vfpv3-d16/custom/selinux-policy-myfork_2020-10-19-4b8d8c06_all.ipk procd-selinux busybox-selinux -busybox"
...
in [selinux-policy](https://git.defensec.nl/?p=selinux-policy.git;a=tree;f=doc;h=895d86cb8411db19eaf52f3cf8ee192eefcf9be5;hb=HEAD)
but if you need assistance and have any questions related to OpenWrt
...
would like the script to be contained and we would to apply the
principle of least privilege to this script executing.
...
...
```
The compiled `policy.31` result and `file_contexts` file in found
...
```
Now it is time to test but before we do will will clear the kernel
ring buffer so that we do not get confused by any "avc denials"
triggered by use copying the policy.31 and file_contexts files over,
because SELinux would not have allowed those operations if it were
...
command to see what permissive would have been denied if we would
...
indicated that the process has all the permissions it needs to
function.
...
`file_contexts` files, reload policy, clear the ring buffer, flash
...
push the policy GitHub, and use that to build a new `ipk` package, and
then create a new sysupgrade image with our new policy using the
`Image Builder`.
...
We just have to adjust two things:
...
```
Now that we have an updated  package we can enclose with our images
...
closely resembles your and then compare and contrast the two to find
way to improve it. If you need help, feel free to ask.

You can find the repository I used for this example at [Github](https://github.com/doverride/selinux-policy-myfork)
